...
filename    :       MNISTFigrueConstruction.py
...
if VISDOM:
    ...
    vis.line(X=torch.Tensor([0]),Y=torch.Tensor([0]),win='acc', \
...
for epoch in range(EPOCH):
    for step, (x, y) in enumerate(train_loader):   
        ...
        if VISDOM:
            vis.line(X=torch.Tensor([epoch * len(train_data) // BATCH_SIZE + step]), \
                     Y=torch.Tensor([acc * 100]),  win='acc',  update='append')
            vis.line(X=torch.Tensor([epoch * len(train_data) // BATCH_SIZE + step]),\
